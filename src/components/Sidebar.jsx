    ...
    <div className='ml-3 h-screen md:overflow-hidden overflow-auto pb-10'>
      {activeMenu && (<>
        <div className='flex justify-between items-center'>
          <Link to='/' onClick={ handleCloseSidebar } className='items-center gap-3 ml-3 mt-4 flex text-xl tracking-tight font-extrabold dark:text-white text-slate-900' >
        ...
        <div className='mt-10 '>
          {links.map((item) => (
            <div key={item.title}>
              ...
              {item.links.map((link) => (
                <NavLink to={`/${link.name}`} key={link.name} onClick={ handleCloseSidebar } className={({ isActive }) => isActive ? activeLink : regularLink}>
                  {link.icon} 
